...
def discrete_kernel(
    ...
    with torch.no_grad():
        ...
        k = cubic_contribution(r).reshape(-1, 1)
...
def reshape_tensor(x: torch.Tensor, dim: int, kernel_size: int) -> torch.Tensor:
    ...
    unfold = F.unfold(x, k)
    unfold = unfold.reshape(unfold.size(0), -1, h_out, w_out)
    return unfold
...
def reshape_output(
    ...
    if b is not None:
        x = x.reshape(b, c, rh, rw)        # 4-dim
    else:
        if c is not None:
            x = x.reshape(c, rh, rw)       # 3-dim
        else:
            x = x.reshape(rh, rw)          # 2-dim
...
def resize_1d(
    ...
    if dim == 2 or dim == -2:
        sample = unfold[..., base, :]
        weight = weight.reshape(1, kernel_size, sample.size(2), 1)
    else:
        sample = unfold[..., base]
        weight = weight.reshape(1, kernel_size, 1, sample.size(3))
...
if __name__ == '__main__':
    ...
    a = torch.arange(64).float().reshape(1, 1, 8, 8)
    z = imresize(a, 0.5)
    print(z)
